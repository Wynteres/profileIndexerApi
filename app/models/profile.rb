class Profile < ApplicationRecord
  ...
  validates :name,
            ...
            format: { with: /^[a-zA-Z]+(([',. -][a-zA-Z ])?[a-zA-Z]*)*$/,
                      multiline: true, message: 'O nome deve conter apenas letras' },
  ...
  validates :twitter_url,
            ...
            format: { with: %r{(?:http(?:s)?://)?(?:www\.)?(twitter|cutt)\.(com|ly)/([a-zA-Z0-9_]+)},
  ...
  validates :twitter_username,
            allow_blank: true,
            length: { maximum: 15,
                      too_long: 'Seu nome do twitter deve ter no máximo %{count} caracteres' }

  validates :twitter_description,
            allow_blank: true,
            length: { maximum: 160,
                      too_long: 'Sua descrição do twitter deve ter no máximo %{count} caracteres' }
end
